...
img =Image.open(r'/Users/nadimsaad/Desktop/IRONHACK/Module 2/Week 5/Project 5/attrition.jpg')
...
if st.checkbox('show/hide data table'):
    ...
    kpi2.metric(label = 'Total number of active employees' , value = 1233)
    kpi3.metric(label = 'Employees Attrition' , value = 237)
...
#################### scatter Logic - relevant to attrition ##################################
if st.checkbox('Click here to Scatter Chart, relevant to the attrition'):
...
if st.checkbox('Click here to Bar Chart, relevant to the attrition'):
...
################# Histogram Logic -relevant to all company ########################
if st.checkbox('Click here to Histogram, relevant to the whole company'):
    st.sidebar.markdown("### Histogram: Explore caracteristics of the whole company : ")
    #name of columns in lists
    measurements = IBM.drop(labels=["Attrition"], axis=1).columns.tolist()
    ...
    if hist_axis:
        ...
        st.pyplot(hist_fig)
#################### Hexbin Chart Logic - relevant to all company##################################

if st.checkbox('Click here to Hexbin Chart, relevant to the whole company'):
    st.sidebar.markdown("### Hexbin Chart: Explore company caracteristics:")
    #name of columns in lists
    indexbarshart=['Age', 'DistanceFromHome', 
     'Education',
...
if st.checkbox('Click here to select information on ATTRITION'):
    ...
    startangle=5, shadow=True, legend = False, fontsize=10)
        ...
        ay.set_xlabel("")
        ay.set_ylabel("")
        ay.set_title('Marital Satus, for the ones who left company')
        st.pyplot(fig4) 
        
        
        #WorkLifeBalance, for the ones who left company
        ...
        ab.set_xlabel("")
        ab.set_ylabel("")
        ab.set_title('Work Life Balance, for the ones who left company')
        st.pyplot(fig9)
